  ...
  return (
    <>
      <div className="hidden md:block fixed bottom-6 left-1/2 transform -translate-x-1/2 z-50">
          ...
          <div className="flex items-center justify-between space-x-2">
            {menuItems.map((item, index) => (
              <motion.button
                ...
                className={`flex flex-col items-center space-y-1 px-3 py-2 rounded-xl transition-all duration-300 group relative ${activeSection === item.id
        ...
        <motion.div
          className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 w-32 h-8 bg-gradient-to-t from-white/10 to-transparent blur-xl rounded-full"
            ...
            >
              {menuItems.map((item, index) => (
                <motion.button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-300 group ${activeSection === item.id
                      ? 'text-white bg-white/10'
                      : 'text-muted-foreground hover:text-white hover:bg-white/5'
                    }`}
