...
export const CommitsGrid = ({ text }: { text: string }) => {
    ...
    const getDeterministicColor = (index: number) => {
        ...
        // Ensure index is positive and modulo logic is safe
        const rand = Math.abs(Math.sin(index * 43758.5453));
        const randomIndex = Math.floor(rand * commitColors.length) % commitColors.length;
        return commitColors[randomIndex];
    };

    const getDeterministicDelay = (index: number) => {
        const rand = Math.abs(Math.sin(index * 12.9898));
        return rand * 0.6; // Return number for motion
    };

    const getDeterministicFlash = (index: number) => {
        const rand = Math.abs(Math.sin(index * 78.233));
        return rand < 0.3;
    };
    ...
    return (
        ...
        >
            {Array.from({ length: gridWidth * gridHeight }).map((_, index) => {
                ...
                const shouldFlash = !isHighlighted && getDeterministicFlash(index);
                const color = getDeterministicColor(index);
                const delay = getDeterministicDelay(index);

                return (
                    <motion.div
                        key={index}
                        initial={{ backgroundColor: "hsl(var(--card))" }} // Use card color (need to ensure it parses correctly or use 'transparent')
                        animate={
                            isHighlighted
                        ...
                        transition={
                            isHighlighted
                                ...
                                : shouldFlash
                                    ? { duration: 0.6, delay: delay, times: [0, 0.5, 1] }
                                    : {}
                        }
                        className={cn(
                            `border h-full w-full aspect-square rounded-[4px] sm:rounded-[3px] bg-card`,
                            // Remove tailwind animation classes, let motion handle it
                        )}
                    />
                );
            })}
